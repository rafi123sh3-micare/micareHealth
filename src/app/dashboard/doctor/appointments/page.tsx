...
import { Search, Calendar, Clock, Video, CheckCircle, X, FileText, ChevronDown, Upload, Check, Plus, Zap, Printer, Scan, Receipt, Download, Heart } from 'lucide-react';
...
const statusOrder: Record<string, number> = {
  ...
  const handleAddWalkin = async () => {
    ...
    try {
      ...
      if (patientResult.error || !newPatient) { toast.error('রোগী তৈরি করতে ব্যর্থ'); setCreatingWalkin(false); return; }
                        ...
                        <div className="flex items-center justify-end gap-1">
                          {apt.status !== 'completed' && (
                            <>
                              ...
                              <button onClick={() => { setEditInvoiceApt(apt); setEditFeeType(apt.fee_type || 'new'); setEditAdvance(apt.advance || 0); setShowInvoiceEditModal(true); }} className="p-2 text-emerald-500 hover:bg-emerald-50 rounded-lg transition-colors" title="ইনভয়েস"><Receipt className="w-4 h-4" /></button>
                            </>
                          )}
                          {apt.status === 'completed' && (
                            <div className="flex items-center justify-end gap-1">
      ...
      <Modal isOpen={showWalkinModal} onClose={() => setShowWalkinModal(false)} title="নতুন অ্যাপয়েন্টমেন্ট">
        <div className="space-y-5">
          ...
          <div>
            ...
            <div className="grid grid-cols-3 gap-2">
              {FEE_TYPES.map((ft) => (
                <button key={ft.value} type="button" onClick={() => setWalkinPatient({...walkinPatient, fee_type: ft.value})}
                  className={`py-3 px-2 rounded-lg border-2 text-center transition-all text-sm ${walkinPatient.fee_type === ft.value ? 'border-emerald-500 bg-emerald-50 text-emerald-700' : 'border-slate-200 hover:border-slate-300'}`}>
                  <div className="font-semibold">{ft.label}</div>
                  <div className="text-xs mt-0.5 opacity-75">৳{ft.amount}</div>
                </button>
              ))}
